...
def render_admin_view(conn, regras):
    ...
    st.set_page_config(page_title="Banco De Dados Da Dino-Tech", page_icon="🦖", layout="wide")
    inject_canva_css()
    
    # Sidebar com informações do usuário e controles
    with st.sidebar:
        usuario_safe = html.escape(st.session_state.usuario_logado)
        st.markdown(
            f"""
            <div style='display:flex; justify-content:center; pointer-events:none;'>
                <a href='https://www.sesi.portaldaindustria.com.br/para-voce/robotica/first-lego-league-challenge#modal-robo'
                   style='display:inline-block; padding:0; margin:0; border: none; line-height:0; pointer-events:auto;'>
                    <img src='https://cdn.brandfetch.io/idm_V4rm0p/theme/dark/logo.svg?c=1dxbfHSJFAPEGdCLU4o5B'
                         style='display:block; padding:0; margin:0; border:none; width:300px; height:auto;' class='simple-image' />
                </a>
            </div>
            <div class='user-info' style='margin-bottom:1rem; width:100%; text-align:center;'>
                👤 <b>{usuario_safe}</b> (admin)
            </div>
            """,
            unsafe_allow_html=True
        )
        if st.button("🔄 Recarregar", key="recarregar_sidebar_admin", help="Atualizar a página"):
            st.rerun()
        if st.button("🔒 Sair", key="sair_sidebar_admin", help="Fazer logout do sistema"):
    ...
    st.markdown(
        ...
        <div style='text-align:center; margin-bottom:2.5rem;'>
            ...
            <div class='canva-title'>Admin Dashboard</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    aba_selecionada = st.sidebar.radio("Ir para:", abas_admin, label_visibility="collapsed")

    # ==============================================================================
    # ABA 1: Setores
    # ==============================================================================
    if aba_selecionada == "📂 Setores":
        with st.container(border=True):
            st.markdown("## 📂 Gestão de Setores")
    ...
    # ==============================================================================
    if aba_selecionada == "📜 Direitos gerais":
        with st.container(border=True):
    ...
    # ==============================================================================
    if aba_selecionada == "🔑 Direitos por Setor":
        with st.container(border=True):
            ...
            for setor in regras.get("setores", []):
                with st.expander(f"🏗️ {setor['nome']}", expanded=False):
                    ...
                    for idx in range(len(direitos.copy())):
                        ...
                        with col2:
                            if st.button("❌", key=f"del_dir_setor_{setor['nome']}_{idx}", help="Remover este direito exclusivo"):
                                direitos.pop(idx)
                                salvar_regras(conn, regras)
                                st.rerun()
    ...
    # ABA 4: Regras gerais
    # ==============================================================================
    if aba_selecionada == "📑 Regras gerais":
        with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            if integrantes:
                for integrante_id, nome in integrantes:
                    ...
                    if st.button(f"❌ Remover {nome}", key=f"rem_integrante_{integrante_id}", help="Excluir integrante e seu login"):
                    ...
                    if atribs:
                        ...
                        for idx_atrib, (setor, funcao) in enumerate(atribs):
                            ...
                            with col2:
                                if st.button("❌", key=f"rem_atrib_{integrante_id}_{idx_atrib}", help="Remover esta atribuição do membro"):
    ...
    # ABA 7: Editar regras
    # ==============================================================================
    if aba_selecionada == "⚙️ Editar regras":
        with st.container(border=True):
                    ...
                    for func in setor.get("funcoes", []):
                        ...
                        with st.expander(f"✏️ Responsabilidades de {func['nome']}", expanded=False):
                            ...
                            for idx, r in enumerate(resp_list.copy()):
                                ...
                                with col2:
                                    if st.button("❌", key=f"del_resp_{setor['nome']}_{func['nome']}_{idx}", help="Remover esta responsabilidade"):
                                        resp_list.pop(idx)
            ...
            with st.expander("📜 Direitos gerais da equipe", expanded=False):
                ...
                for idx in range(len(direitos.copy())):
                    ...
                    with col1:
                        ...
                        direitos[idx] = novo_d
                    with col2:
                        if st.button("❌", key=f"del_dir_{idx}", help="Remover este direito geral"):
                            direitos.pop(idx)
            ...
            with st.expander("📑 Regras gerais da equipe", expanded=False):
                ...
                for idx in range(len(regras_gerais.copy())):
                    ...
                    with col2:
                        if st.button("❌", key=f"del_reg_{idx}", help="Remover esta regra geral"):
            ...
            with st.expander("💡 Justificativas", expanded=False):
                ...
                for idx, chave in enumerate(chaves):
                    ...
                    with col2:
                        if st.button("❌", key=f"del_just_{idx}", help="Remover esta justificativa"):
                            por_que.pop(chave)
                            salvar_regras(conn, regras)
                            st.rerun()
                ...
                for votacao in votacoes:
                    ...
                    with st.expander(f"{status_emoji} {votacao['titulo']} ({votacao['status'].capitalize()} - {tipo_texto})"):
                        ...
                        if votacao['status'] == 'aberta':
                            if col1.button("🔒 Fechar Votação", key=f"fechar_vot_{votacao['id']}", help="Encerrar votação (ninguém mais poderá votar)"):
                                atualizar_status_votacao(conn, votacao['id'], 'fechada')
                                st.rerun()
                        else:
                            if col1.button("🔓 Reabrir Votação", key=f"reabrir_vot_{votacao['id']}", help="Reabrir votação para novos votos"):
                                atualizar_status_votacao(conn, votacao['id'], 'aberta')
                                st.rerun()
                        if col3.button("🗑️ Excluir Votação", key=f"excluir_vot_{votacao['id']}", type="primary", help="Apagar votação e resultados"):
    ...
    # ==============================================================================
    if aba_selecionada == "👤 Usuários e Senhas":
        with st.container(border=True):
