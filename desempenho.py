...
def excluir_round(conn, round_id):
    """Exclui um round e seus detalhes."""
    try:
        conn.table("robo_round_missoes").delete().eq("round_id", round_id).execute()
        conn.table("robo_rounds").delete().eq("id", round_id).execute()
        return True
    except Exception as e:
        print(f"Erro ao excluir round: {e}")
        return False