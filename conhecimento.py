...
def excluir_topico_treino(conn, treino_id):
    try:
        # Remove progressos associados primeiro para manter integridade (se necessário)
        conn.table("treino_progresso").delete().eq("treino_id", treino_id).execute()
        conn.table("treinamentos").delete().eq("id", treino_id).execute()
        return True
    except:
        return False
